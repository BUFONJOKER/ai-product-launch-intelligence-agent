...
def main():
    """Launch the Streamlit UI for manually running the agent workflow.

    This helper configures a lightweight Streamlit interface that lets a
    user choose a `company_name` and which agent to invoke, then runs
    the compiled workflow and displays the selected agent's output.
