...
async def stream_agent(agent_run: AgentRun, api_key_openai):
    ...
    config = {
        ...
        "run_name": f"{agent_run.company_name}_agent_run_{thread_id}",
    }
    ...
    async with await AsyncConnection.connect(
        ...
        initial_state = AgentState(
            company_name=agent_run.company_name,
            agent_invoke=agent_run.agent_invoke,
...
async def start_agent(agent_run: AgentRun):
    api_key_openai = agent_run.api_key_openai
    api_key_openai = decrypt_key(api_key_openai)
    return StreamingResponse(
        stream_agent(agent_run, api_key_openai), media_type="text/event-stream"
    )
...
async def get_agent_response(thread_id: str, api_key_openai: str):
    api_key_openai = decrypt_key(api_key_openai)
    result = await get_final_response(thread_id, api_key_openai)
